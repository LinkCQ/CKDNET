    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=80):
            ...
            if iteration < start_iter + 10000:
                training['offset_weight'] = 50
            else:
                training['offset_weight'] = 20

            training_loss, focal_loss, size_loss, offset_loss = nnet.train(**training)
            ...
            if display and iteration % display == 0:
                ...
                print("size loss at iteration {}:     {}".format(iteration, size_loss.item()))
                print("offset loss at iteration {}:     {}".format(iteration, offset_loss.item()))
                # print("cls loss at iteration {}:      {}\n".format(iteration, cls_loss.item()))

            del training_loss, focal_loss, size_loss, offset_loss  # , cls_loss

            if val_iter and validation_db.db_inds.size and iteration % val_iter == 0:
                nnet.eval_mode()
                validation = pinned_validation_queue.get(block=True)
                validation['offset_weight'] = 20
