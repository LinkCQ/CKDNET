...
class Config:
    def __init__(self):
        ...
        self._configs["source_dir"] = "tensor"
        self._configs["train_mode"] = "c"
        self._configs["frame"] = 3
    ...
    @property
    def video(self):
        return self._configs["data_dir"].split('/')[-1]
    ...
    @property
    def train_mode(self):
        return self._configs["train_mode"]
    ...
    @property
    def result_dir(self):
        result_dir = os.path.join(self._configs["result_dir"], self.video)
    ...
    @property
    def frame(self):
        return self._configs["frame"]

    @property
    def snapshot_dir(self):
        snapshot_dir = os.path.join(self.cache_dir, "nnet", "{}_{}".format(self.video, self.frame))
    ...
    def snapshot_file(self):
        snapshot_file = os.path.join(self.snapshot_dir, self.train_mode + '-' + self.snapshot_name + "_{}.pkl")
    ...
    @property
    def source_dir(self):
        return self._configs["source_dir"]
    ...
    def set_configs(self):
        cfg_file = os.path.join(self.config_dir, self.cfg_file + ".json")
        with open(cfg_file, "r") as f:
            configs = json.load(f)

        configs["system"]["snapshot_name"] = self.cfg_file
        self.update_config(configs["system"])
